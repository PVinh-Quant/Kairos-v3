> 📖 **[Phiên bản Tiếng Việt / Vietnamese Version](data_flow_diagrams_VI.md)**

# KAIROS v3.2 — Data Flow Diagrams & Workflow (Production & Live Core)

> **Legend:** ✅ = existing | ❌ = to build | 🔗 = reuse existing module | **[DEFER]** = deferred for first release

---

## A. Master Data Flow — Overview
...
flowchart TB
    ...
    subgraph FUNDING["💸 Funding Settlement Rules"]
        ...
        FD2["surprise_funding = realized - expected\nExpected from Premium Index MA"]
        FD3["⚠️ Funding settlement-aligned:\nCharged ONLY at settlement bars (00:00/08:00/16:00 UTC)\nOnly when position[t] != 0 AND position[t-1] != 0\nNo per-bar approximation"]
        FD1 --> FD2 --> FD3
    end
...
flowchart TB
    ...
    subgraph MONITOR["📡 Performance & Health Monitoring"]
        ...
        M2["giam_sat/canh_bao/alert_manager.py ✅\n4-TIER ROUTING:\n  CRITICAL → Telegram 24/7 (no suppression)\n  ALERT → Telegram 00:00-20:00 UTC\n  WARNING → Daily digest 07:00 UTC\n  INFO → Log only\nSuppression: same-source 5 minutes | flood >5/10min\nCool-down 30 minutes after HALT resume"]
...
flowchart TB
    ...
    subgraph LINEAGE["📋 M-D0: Data Lineage ✅\ndong_co_du_lieu/quan_ly_phien_ban/"]
        ...
        LD2["lineage_registry.py\nas_of_feature_snapshot(date) → dataset_id\nAppend-only — no delete or modify"]
        LD3["Every data pipeline MUST log dataset_id_used\n[INV-D0.2] never use 'current data'"]
        LD1 --> LD2 --> LD3
    end
    ...
    subgraph VALIDATE["Validation rules"]
        ...
        V2["[INV-D1.2] No ffill on raw OHLCV"]
        V3["[INV-D1.3] recv_time_ns >= event_time_ns"]
        V4["[INV-D2.2] data_quality ∈ {3,4} NOT used in signal\n(quality>1 → label=NaN)"]
        V5["All timestamps = UTC nanoseconds (no exceptions)"]
        V6["[INV-D1.6] Bar boundary: left-inclusive, right-exclusive\nbar_start_ns <= event_time_ns < bar_end_ns\nEvent at exactly T+1 belongs to bar T+1, not T"]
        V7["[INV-D4.15] schema_version field mandatory\n  in all Parquet files\n[INV-D4.16] Do not hardcode column index\n[INV-D4.17] Required cols: crash fast | Optional: NaN"]
    end
...
```mermaid
flowchart TB
    subgraph DASH["📊 Live Dashboard — Build BEFORE Day 1 of paper trading"]
        direction TB

        subgraph S1["SCREEN 1: HEALTH (daily, ≤60 seconds)"]
            direction TB
        ...
        subgraph S2["SCREEN 2: STRATEGY DETAIL (when issue occurs)"]
            direction TB
        ...
        subgraph S3["SCREEN 3: TCA (weekly)"]
            ...
            T2["adverse_selection actual vs model\n(update exchange_metadata when gap > 30%)"]
